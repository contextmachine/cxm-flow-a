...
export function distinctByKeys<T>(source$: RX.Observable<T>, keys: string[]) {
  return source$.pipe(
    RX.distinctUntilChanged((prev, curr) => {
      return keys.every((key) => prev[key as keyof T] === curr[key as keyof T]);
    })
  );
}
